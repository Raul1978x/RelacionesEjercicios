...
/**
 * 2. Realizar el juego de la ruleta rusa de agua en Java. Como muchos saben, el
...
public class ServicioRevolverAgua {
    ...
    private boolean moja;

    public ServicioRevolverAgua() {
        this.revolverAgua = new RevolverAgua();
        this.moja = false;
    }

    public void llenarRevolver() {
        revolverAgua.setPosicionActual((int) (Math.random() * 6 + 1));
        revolverAgua.setPosicionAgua((int) (Math.random() * 6 + 1));
    }

    public void mojar() {
        if (revolverAgua.getPosicionActual().equals(revolverAgua.getPosicionAgua())) {
            moja = true;
    ...
    public void siguienteChorro() {
        if (!moja) {
            if (revolverAgua.getPosicionActual() > 6) {
                revolverAgua.setPosicionActual(revolverAgua.getPosicionActual() + 1);
            } else if (revolverAgua.getPosicionActual().equals(6)) {
                revolverAgua.setPosicionActual(0);
            }
        }
    }

    public void informacion() {
        System.out.print("La posicion actual del tambor es: " + revolverAgua.getPosicionActual());
        System.out.print("La posicion del agua es: " + revolverAgua.getPosicionActual());
    }
}
